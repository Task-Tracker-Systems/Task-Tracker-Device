...
void lv_ssd1306_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    /* Divide by 8 */
 ...
 *   STATIC FUNCTIONS
 **********************/
static uint8_t i2c_write(uint8_t i2c_address, uint8_t command, uint8_t * data, size_t len)
{
...
static uint8_t send_data(lv_disp_drv_t *disp_drv, void *bytes, size_t bytes_len)
{
    ...
    return i2c_write(OLED_I2C_ADDRESS, data[0], data + 1, bytes_len - 1 );
}
...
static uint8_t send_pixels(lv_disp_drv_t *disp_drv, void *color_buffer, size_t buffer_len)
{
    ...
    return i2c_write(OLED_I2C_ADDRESS, OLED_CONTROL_BYTE_DATA_STREAM, (uint8_t *)color_buffer, buffer_len);
}
